...
//     native DzFrameSetScriptByCode takes integer frame, integer eventId, code funcHandle, boolean sync returns nothing
//离开
function yh___Leave_Actions takes nothing returns nothing
	call TriggerExecute(yh_Leave)
endfunction
//进入
function yh___Enter_Actions takes nothing returns nothing
	call TriggerExecute(yh_Enter)
endfunction
//注册Frame UI事件回调
function yh_FrameSetScriptByCode takes nothing returns nothing
	if yh_integer1 == 2 then // 鼠标进入
call DzFrameSetScriptByCode(yh_frame1, 2, function yh___Enter_Actions, false)
	elseif yh_integer1 == 3 then // 鼠标离开
call DzFrameSetScriptByCode(yh_frame1, 3, function yh___Leave_Actions, false)
	else
