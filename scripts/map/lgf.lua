...
gTrg.RegUserPlayerUnitEvent(EVENT_PLAYER_UNIT.HERO_LEVEL, function()
    ...
    if Hero[trgP] == trgU then
        ...
        printF(GetUnitName(trgU) .. I2S(Lev))
        -- 触发单位满足等级
        if Lev % 10 == 9 then
...
function Actions_C_1(whichHero)
    ...
    printF(GetUnitName(whichHero) + I2S(Lev))
    -- 判断等级是否满足条件
    if Lev % 9 ~= 9 then
    ...
    gH.setUpHeroLevel(whichEnemy, Lev * 10, false)
    ...
    TimerStart(CreateTimer(), 2.00, true, function()
        ...
        if not gU.is_alive(whichEnemy) then
            ...
            gH.setUpHeroLevel(whichHero, GetHeroLevel(whichHero) + 1, true)
            -- 传送挑战选手
...
gTrg.RegTimerEvent(0.50, true, function()
    ...
    while gGroup.getCount(group) > 0 do
