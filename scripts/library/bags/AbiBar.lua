...
function Enter_Actions()
    UI_State = "技能栏"
    UI_Num = LoadInteger(UI_Hash, gDz.F2I(gDz.GetTriggerUIEventFrame()), 0)
    ActionFrame = gDz.GetTriggerUIEventFrame()
end
    ...
    for i = 0, 3 do
        for i = 0, 3 do
            if GetLocalPlayer() == GetLocalPlayer() then
                frame = gDz.FrameGetCommandBarButton(lopA, lopB)
                SaveInteger(UI_Hash, gDz.F2I(frame), 0, 4 * lopA + lopB + 1) -- 关联序号
